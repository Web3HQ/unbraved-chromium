...
void MigrationCallback(InitializeCallback callback,
  ...
  for (const bool success : results) {
    if (!success) {
      ...
      // next startup. All saves are idempotent, so retrying is safe.
...
void LoadConfirmationStateCallback(std::optional<WalletInfo> wallet,
  ...
  const ConfirmationTokenList confirmation_tokens =
      json::reader::ParseConfirmationTokens(*json, *wallet)
          .value_or({} /*ConfirmationTokenList*/);
  ...
  const PaymentTokenList payment_tokens =
      json::reader::ParsePaymentTokens(*json).value_or({} /*PaymentTokenList*/);
