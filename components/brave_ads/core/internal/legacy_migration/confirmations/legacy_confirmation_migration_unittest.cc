...
size_t GetConfirmationTokenCount() {
  base::test::TestFuture<bool, ConfirmationTokenList> future;
  database::table::ConfirmationTokens().GetAll(future.GetCallback());
  const auto [success, tokens] = future.Take();
  EXPECT_TRUE(success);
  return tokens.size();
}

size_t GetPaymentTokenCount() {
  base::test::TestFuture<bool, PaymentTokenList> future;
  database::table::PaymentTokens().GetAll(future.GetCallback());
  const auto [success, tokens] = future.Take();
  EXPECT_TRUE(success);
  return tokens.size();
}
...
TEST_F(BraveAdsLegacyConfirmationMigrationTest, MigrateWithMultipleTokens) {
  ...
  MigrateConfirmationState(test::Wallet(), callback.Get());
...
TEST_F(BraveAdsLegacyConfirmationMigrationTest, DoNotMigrateWithoutWallet) {
  ...
  base::MockCallback<InitializeCallback> callback;
  EXPECT_CALL(callback, Run(/*success=*/true));
  MigrateConfirmationState(/*wallet=*/std::nullopt, callback.Get());
