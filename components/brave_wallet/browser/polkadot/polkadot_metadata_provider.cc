...
void PolkadotMetadataProvider::OnGetMetadata(
  ...
  const size_t metadata_size =
      metadata_bytes_or_error.has_value() ? metadata_bytes_or_error->size() : 0u;
  LOG(ERROR) << "XXXZZZ metadata rpc response chain_id=" << chain_id
             << " has_err="
             << (metadata_bytes_or_error.has_value() ? "false" : "true")
             << " has_result="
             << (metadata_bytes_or_error.has_value() ? "true" : "false")
             << " result_len=" << metadata_size
             << " err="
             << (metadata_bytes_or_error.has_value()
                     ? ""
                     : metadata_bytes_or_error.error());

  auto parsed = ParseChainMetadataFromHex(std::move(metadata_bytes_or_error));
  if (!parsed.has_value()) {
    const bool parser_failed = parsed.error() == "Failed to parse runtime metadata.";
