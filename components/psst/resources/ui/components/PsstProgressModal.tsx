...
export default function PsstProgressModal(_props: Props) {
  ...
  const { data: settingsData } = browserProxy?.useSettingsCardData?.() || {
    data: null,
  }
  const { data: requestStatus } = browserProxy?.useRequestStatus?.() || {
    data: null,
  }
  const { data: completionStatus } = browserProxy?.useCompletionStatus?.() || {
    data: null,
  }
  const { mutate: applyChanges } = browserProxy?.useApplyChanges?.() || {
    mutate: () => {},
  }
